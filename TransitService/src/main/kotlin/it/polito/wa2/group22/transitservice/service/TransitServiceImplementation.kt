...
import it.polito.wa2.group22.transitservice.entity.Transit
import it.polito.wa2.group22.transitservice.entity.toDTO
...
import it.polito.wa2.group22.transitservice.service.TransitService
...
@Service
class  TransitServiceImplementation : TransitService {
    ...
    override suspend fun getRepTransits(datarange: TimePeriodDTO): StatisticsDTO {
    ...
    override suspend fun getUserRepTransits(datarange: TimePeriodDTO, username: String): StatisticsDTO {
        val transitsCounter = transitRepository.findAllTransit().filter {
            it.ticket_user == username &&
                    it.transit_date.isAfter(LocalDateTime.parse(datarange.start_date, formatter)) &&
                    it.transit_date.isBefore(LocalDateTime.parse(datarange.end_date, formatter))
        }.count()
        if(transitsCounter == 0){
